...
def create_all_visualizations(results, X_test_df, y_test, label_names):
    ...
    try:
        ...
        for idx, model_name in enumerate(model_names):
            if idx >= 3:  # Hanya tampilkan maksimal 3 model
                break
        ...
        print(f"⚠️  Error creating confusion matrices: {e}")
    ...
    try:
        ...
        print(f"⚠️  Error creating feature importance: {e}")
    
    # 4. RECALL PER CLASS COMPARISON
    try:
        if results and len(label_names) > 0:
        ...
        print(f"⚠️  Error creating recall per class: {e}")
    ...
    try:
        ...
        print(f"⚠️  Error creating performance summary: {e}")
    
    # Tutup semua figure untuk hemat memory
    plt.close('all')
...
def save_models_to_csv_pkl(results, scaler, X_test_df, y_test, label_names):
    ...
    try:
        ...
        # Periksa apakah scaler sudah fit
        ...
        try:
            ...
            if name == 'Decision Tree':
                ...
                model_info = pd.DataFrame({
                    ...
                    'value': [
                        getattr(model, 'n_neighbors', 5),
                        getattr(model, 'weights', 'uniform'),
                        getattr(model, 'metric', 'minkowski'),
                        getattr(model, 'algorithm', 'auto')
                    ]
                })
                ...
                model_info = pd.DataFrame({
                    ...
                    'value': [
                        getattr(model, 'C', 1.0),
                        getattr(model, 'max_iter', 100),
                        getattr(model, 'solver', 'lbfgs'),
                ...
                if hasattr(model, 'coef_') and model.coef_ is not None:
                    ...
                    if len(coef_array.shape) == 1:
                        # Jika hanya 1 class (binary classification)
                        ...
                        coefficients = pd.DataFrame(
                            coef_array,
                            columns=['temperature', 'humidity', 'hour', 'minute'],
                            index=[f'class_{i}' for i in range(coef_array.shape[0])]
                        )
                ...
                # Model lainnya
                model_info = pd.DataFrame({
                    'parameter': ['model_type', 'n_features', 'n_classes'],
                    'value': [
                        name,
                        getattr(model, 'n_features_in_', 0) if hasattr(model, 'n_features_in_') else 0,
                        getattr(model, 'n_classes_', 0) if hasattr(model, 'n_classes_') else 0
                    ]
                })
    ...
    try:
        ...
        print(f"⚠️  Error saving test data: {e}")
...
def main():
    ...
    try:
        ...
        print("   3. Skip - Just train models")
        ...
        if choice == '1':
            ...
            print("⚠️  Invalid choice, using option 2 (existing CSV data)...")
        ...
        if hivemq_manager.connect():
            ...
            print("⚠️  Cannot connect to HiveMQ for publishing. Skipping...")
        ...
        print("\n✅ All models ready for deployment with HiveMQ integration!")
        print("="*60)
        ...
        # Disconnect HiveMQ
        if hasattr(hivemq_manager, 'connected') and hivemq_manager.connected:
            hivemq_manager.disconnect()
        print("\n👋 Program finished")
